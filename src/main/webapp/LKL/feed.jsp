...
<body>
	...
	<div class="아이템들">
		...
		<div class="아이템">	
			<div class="아이템 썸네일">
		    	<img class="썸네일 이미지" src="${f.f_img }" >
			</div>
	    	<div class="아이템 정보">	
    			<div class="아이템 제목">${f.f_title }</div>
    			<div class="아이템 가격">${f.f_price } &#8361;</div>
    			<div class="아이템 브랜드">${f.f_brand }</div>
    		</div>
    	</div>	
		</c:forEach>
	</div>
	...
	<!-- 페이지 처리 -->
	<a href="FeedPageC?p=1">[맨처음]</a>
	<c:choose>
		<c:when test="${curPageNo != 1 }">
		<a href = "FeedPageC?p=${curPageNo -1 }"><button>◀</button></a>
		</c:when>
			<c:otherwise>
	...
	<a href="FeedPageC?p=${pageCount }">[맨끝]</a>
