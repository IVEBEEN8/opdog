	...
	<div class="shop-main">
		...
		<div class="search-price">
			<div class="search">
				...
				<div class="search-img">
					<img alt="" src="2_shop/img/Search.png"
						onclick="location.href='SnackSearchC?search='+document.getElementById('search').value">
				</div>
			</div>
			...
			<div class="price-order" align="right">
				<label class="price-label" for="radio-high-price"><input type="radio" name="sort" value="high" id="radio-high-price"
					onclick="location.href='SnackSortC?sort=high'"> Highest
				price &nbsp;</label> 
				<label class="price-label" for="radio-low-price"><input type="radio" name="sort" value="low" id="radio-low-price"
					onclick="location.href='SnackSortC?sort=low'"> Lowest price</label>
		...
		<div class="items-container">
			<div id="items" class="items">
				<%-- <c:forEach varStatus="" var="s" items="${snacks }">
					<a href="SnackDetailC?no=${s.s_no }">
						<div class="item">
							<div class="아이템 썸네일">
								<img class="item-img" src="${s.s_img }">
							</div>
							<div class="item-info">
								<div class="item-brand" align="left">${s.s_brand }</div>
								<div class="item-title" align="left">${s.s_title }</div>
							</div>
							<div class="item-line-price">
								<div class="item-line">
									<div id="item-line"></div>
								</div>
								<div class="item-price" align="left">${s.s_price }&#8361;</div>
							</div>
						</div>
					</a>
				</c:forEach> --%>
		...
		<script type="text/javascript">
			$(document)
					.ready(
							function() {
								var dataSource = ${jsonSnacks}; // 데이터 소스, 예: 페이지 번호의 배열
														...
														$
																.each(
																		...
																		function(
																				index,
																				item) {
																			dataHtml += '<a href="SnackDetailC?no='
																					+ item.s_no
																					+ '">';
																			dataHtml += '<div class="item">';
																			dataHtml += '<div class="아이템 썸네일">';
																			dataHtml += '<img class="item-img" src="' + item.s_img + '">';
																			dataHtml += '</div>';
